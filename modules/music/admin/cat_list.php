<?php
    ...
    if($nv_Request->isset_request("action","post,get")){
        ...
        if($id > 0 and $checksess == md5($id.NV_CHECK_SESSION)){
            $db ->query("DELETE FROM ".NV_PREFIXLANG."_music_cats WHERE id=".$id);
        }
    }
    ...
    if (!empty($array_row)) {
        ...
        foreach($array_row as $row){
            ...
            . NV_OP_VARIABLE . '=cat_content&amp;id=' . $row['id'];
            $row['url_delete'] = NV_BASE_ADMINURL .'index.php?' . NV_LANG_VARIABLE . '=' 
            . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=cat_list&amp;id=' . $row['id']."&action=delete&checksess=" . md5($row['id'].NV_CHECK_SESSION);
            $xtpl->assign('ROW',$row);
