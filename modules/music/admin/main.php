<?php
    ...
    if($nv_Request->isset_request("change_weight","post,get")){
        ...
        if($id > 0 and $new_weight > 0){
            $sql = "SELECT id, weight FROM nv4_vi_music_songs WHERE id !=" .$id;
            $result = $db ->query($sql);
            $weight = 0;
            while($row = $result ->fetch()){
                ...
                $exe = $db ->query("UPDATE nv4_vi_music_songs SET weight =". ($weight) . " WHERE id=" .$row['id']);
            }
            $exe = $db ->query("UPDATE nv4_vi_music_songs SET weight =". $new_weight . " WHERE id=" .$id);
        }
    ...
    if($nv_Request->isset_request("action","post,get")){
        ...
        if($id > 0 and $checksess == md5($id.NV_CHECK_SESSION)){
            $db ->query("DELETE FROM ".NV_PREFIXLANG."_music_songs WHERE id=".$id);
        }
    }
    ...
    if($nv_Request->isset_request("change_active","post,get")){
        $id = $nv_Request ->get_int("id","post,get",0);
        $sql = "SELECT id, active FROM nv4_vi_music_songs WHERE id=" .$id;
        $result = $db ->query($sql);
        if($row = $result ->fetch()){
            $active = $row['active'] == 1 ? 0 : 1;
            $exe = $db ->query("UPDATE nv4_vi_music_songs SET active =". $active . " WHERE id=" .$id);
            if($exe){
                die("OK");
            }
        }
    ...
    $db->sqlreset()
    ->select('COUNT(*)')
    ->from(NV_PREFIXLANG . "_" . $module_data . "_songs");
    $sql = $db->sql();
    ...
    if (!empty($array_row)) {
        ...
        foreach($array_row as $row){
            ...
            $row['url_edit'] = NV_BASE_ADMINURL .'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=song-content&amp;id=' . $row['id'];
